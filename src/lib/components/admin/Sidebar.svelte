<script lang="ts">
	...
	const navItems = [
		...
		{ href: '/admin/vehicles', label: 'Vehicles', icon: 'M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10 M17 16V8a1 1 0 00-1-1h-2.5' },
		...
		{ href: '/admin/chat', label: 'Chat Logs', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' },
...
<aside class="bg-primary h-full flex flex-col {collapsed ? 'w-16' : 'w-60'} transition-all duration-200">
	<!-- Logo -->
	<div class="p-4 border-b border-white/10">
		{#if collapsed}
			<span class="text-white font-heading font-bold text-lg block text-center">NH</span>
		{:else}
			<span class="text-white font-heading font-bold text-lg">New Hope Motors</span>
			<span class="text-white/50 text-xs block">Admin Panel</span>
		{/if}
	</div>
	...
	<div class="p-4 border-t border-white/10">
		<form method="POST" action="/admin/logout">
			<button type="submit" class="flex items-center gap-3 w-full text-sm text-white/60 hover:text-white transition-colors">
				<svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
